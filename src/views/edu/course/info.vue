...
import Tinymce from '@/components/Tinymce'  //引入组件
    ...
    created() {
        ...
        if(this.$route.params && this.$route.params.id){
          ...
          this.init()
          this.getListTeacher()    //初始化所有讲师,一定要加this
          this.getOneSubject()     //初始化一级分类
          beforeAvatarUpload()
        }
          ...
          .then(response =>{
            ...
            subject.getSubjectList()
              .then(response =>{
                ...
                //3.把所有一级分类数组进行遍历，比较当前courseInfo里面一级分类id和所有的一级分类id是否相同
                for(var i=0; i<this.subjectOneList.length; i++){
                    //获取每个一级分类
                    var oneSubject = this.subjectOneList[i]
                    //比较当前courseInfo里面一级分类id和所有的一级分类id是否是否相同
